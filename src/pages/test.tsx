...
import { TwitterLogo, GithubLogo, InstagramLogo } from 'components/icons'
...
export default function Test() {
  ...
  return (
    <>
      <Layout>
        {/* head start */}
        <div className="flex z-40 items-center content-between space-x-2 -ml-6 -mt-6 -mr-6 sm:-ml-4 sm:-mt-4 sm:-mr-4 p-2 sm:p-1 bg-violet-400/25 opacity-0 hover:opacity-100 rounded-none md:rounded-full transition-all duration-150 backdrop-blur-lg">
          <div className="flex">
            <Link href="/">
...
function Example() {
  return (
    <Disclosure defaultOpen>
      {({ open }) => (
        <>
          <Disclosure.Button as="div" className="relative w-full mx-0 mt-0 cursor-pointer group">
            <div
              className={`${
                open ? 'bg-black/0' : 'bg-black/10'
              } absolute inset-0 -ml-3 -mr-3 rounded-2xl transition-all duration-300`}
            />
            <div className="flex w-full justify-between items-center">
              <h3 className="text-3xl py-3">socials</h3>
              <ChevronDownIcon
                className={`${
                  open ? '-scale-100' : ''
